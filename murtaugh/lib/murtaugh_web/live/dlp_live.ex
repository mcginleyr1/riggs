defmodule MurtaughWeb.DlpLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(
        Murtaugh.PubSub,
        Murtaugh.Topics.dlp(socket.assigns.current_org.id)
      )
    end
  ...
  def handle_info({:dlp_event, event}, socket) do
    ...
    {:noreply,
     assign(socket, blocks_today: socket.assigns.blocks_today + 1, recent_events: events)}
  end
  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <.link
          navigate={~p"/orgs/#{@org_slug}/dlp/policies"}
          class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg"
        >
          Manage Policies
        </.link>
      </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="py-2 px-4">
                  <.link
                    navigate={~p"/orgs/#{@org_slug}/agents/#{event.agent_id}"}
